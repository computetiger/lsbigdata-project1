...
# 1번 그룹은 어떤 종류로 예측하나요?
# 2번 그룹은 어떤 종류로 예측하나요?
y_hat1=df.query("x < 45")['y'].mode()
y_hat2=df.query("x >= 45")["y"].mode()

# 각 그룹 entropy는 얼마 인가요?
...
def my_entropy(x):
    ...
    en1=-sum(p_1*np.log2(p_1))
    p_2=df.query(f"x1>={x}")['y'].value_counts() / len(df.query(f'x1>={x}')['y'])
    en2=-sum(p_2*np.log2(p_2))
    return float((en1* n1 + en2 * n2)/(n1+n2))
