...
for col in quant_selected:
    house_train[col].fillna((house_train[col].mean()+house_train[col].median())/2 , inplace=True)
...
for col in quant_selected:
    house_test[col].fillna((house_train[col].mean()+house_train[col].median())/2, inplace=True)
...
for col in qual_selected:
    house_test[col].fillna/(house_train[col].mode()[0], inplace=True)
...
# # csv 파일로 내보내기
sub_df.to_csv("sample_submission11.csv", index=False)
...
# Voting Classifier (Soft voting) 설정
...
print(f'Voting Classifier 정확도: {accuracy:.2f}')